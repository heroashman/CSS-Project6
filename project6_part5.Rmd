...
ps_match_sim <- function(save_model = FALSE){ 
  ...
  lm_model <- lm(as.formula(paste('college ~', paste(covar_sample, collapse='+'))), 
...
ps_match_sim <- function(save_model = FALSE){ 
  ...
  match_model <- matchit(formula = as.formula(paste('college ~', paste(covar_sample, collapse='+'))),
                         data = d %>% 
                           select_at(c("college", "student_ppnscal", covar_sample)) %>%
...
sim_test <- replicate(5, ps_match_sim(), simplify = TRUE)
```
