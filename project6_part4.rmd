...
'''test <- sapply(lapply(covar_sample, function(x){
 suppressWarnings(matchit(formula = as.formula(paste('college ~', paste(x, collapse='+'))),
                         data = d, method = "nearest", distance = "glm", link = "logit", 
                         estimand = "ATT", replace = TRUE, ratio = 3))
}), FUN = testfuns)
...
#run match it 
suppressWarnings(match_model <- lapply(covar_sample, function(x)
  matchit(formula = as.formula(paste('college ~', paste(x, collapse='+'))),
                         data = d, method = "nearest", distance = "glm", link = "logit", 
                         estimand = "ATT", replace = TRUE, ratio = 3)
  ))

   saveRDS(match_model, "part4_ps_simulation_matchmodels.rds")

#calculate ATT 
  #match_data <- lapply(match_model, FUN = match.data)
  
  lm_model <- mapply(function(x,y) 
    lm(as.formula(paste('student_ppnscal ~ college +', paste(x, collapse='+'))), 
                  data = match.data(y), weights = weights), 
    covar_sample, 
    match_model, 
    SIMPLIFY = FALSE
    )
...
# grabs ATT and p-value
  ests <- sapply(lm_model, function(x) summary(x)$coefficients["college", ])


# proportion of covariates that meet the standardized mean difference <= 0.1
share_bal <- sapply(match_model, function(x) bal.tab(x, thresholds = 0.1)$Balance %>%
    transmute(balanced = 1*(M.Threshold == "Balanced, <0.1")) %>%
    unlist() %>%
    mean())

 # mean percent improvement in the standardized mean difference
 pct_improv <- sapply(match_model, function(x) summary(x)$reduction[,1] %>% mean())
...
#results <- matrix(, nrow = 10000, ncol = 5)
...
We run the above simulation 10,000 times and create a dataframe with the returned results.  
...
if (RUNSIM == TRUE) {
  ...
  # run 10,000 times
  sim_results <- replicate(5000, ps_match_sim(), simplify = TRUE)
  ...
  # output so don't need to re-run simulation 
  saveRDS(sim_df, "part4_ps_simulation_results.rds")
}
if (RUNSIM == FALSE){
  
  sim_df <- readRDS("part4_ps_simulation_results.rds")
  #sim_df_2 <- readRDS("part4_ps_simulation_results_2.rds")
  #sim_df <- cbind(sim_df, sim_df2)
...
The plot below shows the share of covariates that meet the balance threshold of 0.1 against the estimate for the average treatment on the treated. In general, as the number of balanced covariates increases, the estimated ATT increases. If we take balanced covariates to be a sign that matching has resulted in "as good as random assignment", this could indicate that the causal effect of college on political participation is actually larger. The plot does not show convergence of ATT like in the K&P paper, but this might be due to our lower number of simulations.  Our "true" model sits pretty squarely in the middle of the distribution of the share of balanced covariates.  
...
#sim_df %>%
  ...
  theme_bw() +
  labs(title = "ATT again Mean Percent Improvement",
       subtitle = "'True' model shown in red") +
  xlab("Percent") +
  ylab("Average Treatment on the Treated") +
...
Finally, we show the number of covariates against the ATT. This plot does suggest convergence of the ATT as the number of covariates is increased.   
...
#sim_df %>%
  ...
  theme_bw() +
  labs(title = "ATT again Number of Covariates",
       subtitle = "'True' model shown in red") +
  xlab("Number") +
  ylab("Average Treatment on the Treated") +
