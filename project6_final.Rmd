...
results <- apply(randomizations, 2, function(x) 
  d %>% filter(x == 0) %>% select(parent_OwnHome) %>% sum() /
    d %>% filter(x == 1) %>% select(parent_OwnHome) %>% sum())

plot(density(results), main = "Density")
```
...
Plotting the percent improvement in the simulated models based on genetic matching similarly reveals that genetic matching consistently performs somewhat worse than ordinary nearest-neighbor matching, with a median percent improvement of 34.4%, for genetic matching compared to 54.6% for nearest-neighbor. Again, both performed significantly better than the body of simulations including any number of covariates, for which the median percent improvement was -5.84%.
